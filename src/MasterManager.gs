...
function getMasterItems() {
  ...
  if (!sheet) { initMasterSheet(); sheet = ss.getSheetByName(MASTER_SHEET_NAME); }
  ...
  data.forEach(function(row, i) {
    if (row[3] === true || row[3] === 'TRUE') {
      result.push({ rowIndex: i + 2, type: row[0], amount: Number(row[1]) || 0, description: row[2] || '' });
    }
  });
  return result;
}
...
function addMasterItem(type, amount, description) {
  if (!type || !amount) return { success: false, message: '種別と金額は必須です' };
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var sheet = ss.getSheetByName(MASTER_SHEET_NAME);
    if (!sheet) { initMasterSheet(); sheet = ss.getSheetByName(MASTER_SHEET_NAME); }
...
function updateMasterItem(rowIndex, type, amount, description) {
  if (!rowIndex || !type || !amount) return { success: false, message: '行番号・種別・金額は必須です' };
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var sheet = ss.getSheetByName(MASTER_SHEET_NAME);
